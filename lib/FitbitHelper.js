...
export default class FitbitHelper {
	...
	static async updateAllTokens(userID, fitbitID, accessToken, refreshToken) {
		let success = await updateAllFitbitTokens(
			userID,
			fitbitID,
			accessToken,
			refreshToken
	...
	static async updateFitbitUserCredentials(userID, authCode) {
		const authResult = await FitbitHelper.getAuthorizationInformation(
			authCode
		)
			.then((responseData) => {
				console.log(
